...
export class UserService {
  ...
  deleteUserRole(id:number): Observable<any>{
    ...
    getEmployeeDepartments(): Observable<any>{
      return this.http.get<any>(`${environment.apiUrl}/employee-departments`)
    }
...
// Employee API's
  ...
  getEmployeeContract(){
    return this.http.get(`${environment.apiUrl}/contractors`);
  }
  ...
  getCustomerWalletList(page:number, size:number, sort:string, dir:string, searchTerm:string): Observable<any>{
   ...
   if(roll === 'CUSTOMER'){
    return this.http.get<any>(`${environment.apiUrl}/customer-wallet-history-list?id=${id}&page=${page}&size=${size}&sortByField=${sort}&sortBy=${dir}&search=${searchTerm}&fromDate=${fromDate}&toDate=${toDate}`);
   } else {
    return this.http.get<any>(`${environment.apiUrl}/customer-wallet-history-by-walletId?walletId=${id}&page=${page}&size=${size}&sortByField=${sort}&sortBy=${dir}&search=${searchTerm}&fromDate=${fromDate}&toDate=${toDate}`);
   }
   }
...
changeViewStatus(id:number): Observable<any>{
  return this.http.get<any>(`${environment.apiUrl}/employee-notification-enabled?id=${id}`)
}
...
postLeave(data:any){
  return this.http.post(`${environment.apiUrl}/apply-employee-leave`,data)
}

editLeave(id:number,data:any){
  return this.http.put(`${environment.apiUrl}/approve-employee-leave/${id}`,data)
}
...
deleteLeave(id:number){
  ...
  getDailyStatus(page:number, size:number, sort:string, dir:string, searchTerm:string, from:string, to:string): Observable<any>{
    return this.http.get<any>(`${environment.apiUrl}/employee-daily-status-view?page=${page}&size=${size}&sortByField=${sort}&search=${searchTerm}&fromDate=${from}&toDate=${to}&sortBy=${dir}`)
  }

  getDailyStatusByUserId(id:number, page:number, size:number, sort:string, dir:string, from:string, to:string): Observable<any>{
    return this.http.get<any>(`${environment.apiUrl}/employee-daily-status-view?id=${id}&page=${page}&size=${size}&sortByField=${sort}&fromDate=${from}&toDate=${to}&sortBy=${dir}`)
  }

  getDailyStatusById(id:number): Observable<any>{
    return this.http.get<any>(`${environment.apiUrl}/employee-daily-status/${id}`)
  }

  postDailyStatus(data:any){
    return this.http.post(`${environment.apiUrl}/employee-daily-status`,data)
  }

  editDailyStatus(id:number,data:any){
    return this.http.put(`${environment.apiUrl}/employee-daily-status/${id}`,data)
  }

  deleteDailyStatus(id:number){
    return this.http.delete(`${environment.apiUrl}/employee-daily-status/${id}`)
  }
...
getSwipeEntryById(id:number): Observable<any>{
  return this.http.get<any>(`${environment.apiUrl}/employee-swipe-entry/${id}`)
}

postSwipeEntry(data:any){
  return this.http.post(`${environment.apiUrl}/employee-swipe-entry`,data)
}

editSwipeEntry(id:number,data:any){
  return this.http.put(`${environment.apiUrl}/employee-swipe-entry/${id}`,data)
}

deleteSwipeEntry(id:number){
  return this.http.delete(`${environment.apiUrl}/employee-swipe-entry/${id}`)
}
...
getContractorPaymentById(id:number): Observable<any>{
  return this.http.get<any>(`${environment.apiUrl}/employee-contract-payment/${id}`)
}

postContractorPayment(data:any){
  return this.http.post(`${environment.apiUrl}/employee-contract-payment`,data)
}

editContractorPayment(id:number,data:any){
  return this.http.put(`${environment.apiUrl}/employee-contract-payment/${id}`,data)
}

deleteContractorPayment(id:number){
  return this.http.delete(`${environment.apiUrl}/employee-contract-payment/${id}`)
}
...
downloadContractEmployeeExcel(className:any): Observable<HttpResponse<Blob>>{
  return this.http.get<Blob>(`${environment.apiUrl}/download-sample-template?className=${className}`, { observe: 'response', responseType: 'blob' as 'json'});
