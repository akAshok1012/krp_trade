...
export class CrmChartComponent implements OnInit {
  ...
  leadByEmpCountChart(){
    this.chart3 = new Chart("LeadByEmpCountChart", {
      type: 'bar', //this denotes tha type of chart

      data: {
        labels:  this.G_by_emp_countChart.map(d => d.createdBy),
        datasets: [
        {
          label: 'Lead Count',
          data: this.G_by_emp_countChart.map(d => d.leadsCount),
  ...
  getLeadStatusCount(){
      this.analytics.leadStatusCount(this.empId).subscribe((response: any) => {
        if (response) {
  ...
  getData(){
    ...
    this.analytics.leadStatusCreaterCount().subscribe((response: any) => {
      if (response) {
        this.G_by_emp_countChart =  (response.data ? response.data : [])
        this.leadByEmpCountChart();
      }
    });
    ...
    this.analytics.leadDailyCount().subscribe((response: any) => {
      if (response) {
       ...
       this.analytics.getDashboardCountDetails().subscribe((response: any) => {
      if (response) {
        this.isData = false;
      }
    });
